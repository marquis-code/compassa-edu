    ...
    <main class="flex-1 lg:-mt-5">
      <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
        <div class="flex justify-between items-center w-full">
          ...
          <button type="button" @click="showDrawer = true" class="text-white lg:hidden border border-white"><svg
              xmlns="http://www.w3.org/2000/svg" width="23" height="23" viewBox="0 0 24 24" fill="none" stroke="#ffffff"
        ...
        <div class="flex flex-col lg:flex-row items-start lg:items-center gap-4 w-full lg:-mr-10 lg:w-auto -mt-1">
          <!-- Search and Filter Toggle -->
          <div class="flex flex-col w-full space-y-4">
            <!-- Top Bar with Search and Filter Button -->
            <div class="lg:flex space-y-3 lg:space-y-0 items-center gap-4 -mt-10 lg:mt-0">
              <!-- Search Bar -->
              <div class="relative flex-1">
                <input v-model="filters.search" type="text" placeholder="Search files and folders"
                  class="w-full bg-white border-[0.5px] md:w-[500px] outline-none border-gray-200 rounded-lg px-4 py-4 lg:py-3.5 text-sm pl-10">
              ...
              <div v-if="isFilterVisible" class="bg-white border-[0.5px] border-gray-200 rounded-lg p-4">
                <div class="flex items-center justify-between mb-4">
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <button @click="toggleFilters" class="text-gray-400 hover:text-gray-500">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div class="space-y-2">
                    ...
                    <select v-model="filters.category"
                      class="w-full bg-white border-[0.5px] outline-none border-gray-200 rounded-lg px-3 py-2 text-sm">
                      <option value="">All Materials Types</option>
                      <option v-for="category in materialTypeArray" :key="category._id" :value="category._id">
                        {{ category.label }}
                      </option>
                    </select>
                  </div>
            ...
            <div v-if="activeFilterCount > 0" class="flex flex-wrap gap-2">
              <div v-for="(value, key) in filters" v-if="value" :key="key"
                ...
                <button @click="filters[key] = ''" class="p-0.5 hover:text-gray-900">
          ...
          <button type="button" @click="showDrawer = true" class="text-white hidden lg:block border border-white"><svg
              xmlns="http://www.w3.org/2000/svg" width="23" height="23" viewBox="0 0 24 24" fill="none" stroke="#ffffff"
              stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <line x1="4" y1="21" x2="4" y2="14"></line>
              <line x1="4" y1="10" x2="4" y2="3"></line>
      ...
      <div v-if="allMaterialsList?.length && !loading" class="mb-8">
        <h2 class="text-lg font-medium mb-4">Recent Uploads</h2>
        <div v-if="filteredMaterials && !loading"
          ...
          <div v-for="file in filteredMaterials" :key="file._id"
            ...
            <div v-if="file?.fileUrls.length" v-for="item in file?.fileUrls">
              <img :src="getFileIcon(item)" v-if="isIconNeeded(item)" class="object-cover h-32 w-full" />
              <img v-else :src="item" alt="Image File" class="w-32 h-32 object-cover">
            </div>
            <div class="px-4 py-3">
              <div class="w-full flex justify-between items-center">
                <span class="text-xs border rounded-full px-3 py-1.5" :class="{
                ...
                <section class="flex justify-between items-center">
                  ...
                  <div v-if="file?.status === 'approved'" class="pt-4">
                    <div v-for="fileUrl in file.fileUrls" :key="fileUrl" class="mb-4">
                      <button class="text-xs flex items-center justify-center" @click.stop="downloadMaterial(fileUrl)">
      ...
      <section class="max-w-7xl mx-auto mb-10" v-else-if="loading && !allMaterialsList?.length">
                    ...
                    <div class="space-y-6">
                      ...
                      <div>
                        ...
                        <select v-model="payload.category"
                          ...
                          <option v-for="item in categories?.data" :key="item.key" :value="item._id">
                            {{ item.name }}
                          </option>
                        </select>
                      </div>
                    ...
                    <div class="space-y-6">
                      <div>
                        ...
                        <select v-model="payload.academicLevel"
                          ...
                          <option v-for="item in academicLevelArray" :key="item.key" :value="item.key">
                            {{ item.label }}
                          </option>
                        </select>
                      </div>
                      ...
                      <div>
                        ...
                        <select v-model="payload.materialType"
                          ...
                          <option v-for="item in materialTypeArray" :key="item.key" :value="item.key">
                            {{ item.label }}
                          </option>
                        </select>
                      </div>
                  ...
                  <div v-if="currentStep === 3">
                    <div>
                      ...
                      <div @dragover.prevent="dragover = true" @dragleave.prevent="dragover = false"
                        ...
                        <!-- {{payload?.fileUrls}} -->
                        <div v-if="payload?.fileUrls.length > 0" class="max-w-xl grid grid-cols-2">
                          <!-- Map through the uploaded files -->
                          <div v-for="(file, index) in payload?.fileUrls" :key="index" class="w-full mb-4">
                            <img v-if="!['pdf', 'docx'].includes(getFileExtension(file))"
                              class="w-full max-w-32 object-cover rounded-lg" :src="file" />
                            <img v-else src="@/assets/icons/pdf-file.svg" class="w-full h-[250px] object-contain p-8" />
                          </div>
                        </div>
                        <div v-else class="text-gray-400 flex flex-col items-center justify-center p-8">
                          <img src="@/assets/icons/file-upload.svg" class="h-16 w-16 mb-4" />
                          <p class="text-base max-w-sm text-center">
                            Drag and drop your file here, or click to select
                          </p>
                        </div>
                        <input type="file" ref="fileInput" multiple class="hidden" @change="handleFileUpload" required>
                      </div>
                      <!-- Loading spinner -->
...
// Enhanced computed property for filtered materials
const filteredMaterials = computed(() => {
  if (!allMaterialsList.value) return []

  return allMaterialsList.value.filter(material => {
    // Text search filter
    ...
    const matchesCategory = !filters.value.category ||
      ...
      const matchesMaterialType = !filters.value.materialType ||
      material.materialType === filters.value.materialType
